...
function App() {
  ...
  const generateSuggestion = (incomeVal, principalVal, monthlyVal) => {
    ...
    const recommendedMessage = incomeVal > 10000
      ? `คุณมีรายได ${incomeVal.toLocaleString()} บาท แนะนำให้จ่ายเพิ่มพิเศษ ${minSuggested.toLocaleString()} - ${maxSuggested.toLocaleString()} บาท เพื่อปิดหนี้ไวขึ้น`
      : `รายไดของคุณอยู่ที่ ${incomeVal.toLocaleString()} บาท ค่างวดจะถูกปรับตามความสามารถในการชำระ`;
    ...
    <div className="App">
      ...
      <div className="input-group">
        ...
        <input
          ...
          onChange={(e) => {
            const value = e.target.value;
            // ตรวจสอบว่าเป็นตัวเลขและไม่มากกว่าเงินต้น
            if (/^\d*\.?\d*$/.test(value)) {
